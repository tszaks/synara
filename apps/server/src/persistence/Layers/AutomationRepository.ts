...
const AutomationDefinitionDbRow = Schema.Struct({
  ...
  memoryContextMode: AutomationDefinition.fields.memoryContextMode,
...
const makeAutomationRepository = Effect.gen(function* () {
    ...
    execute: ({ id }) =>
      sql`
        SELECT
          ...
          mode,
          COALESCE(memory_context_mode, 'off') AS "memoryContextMode",
    ...
    execute: (definition) =>
      sql`
        ...
        SET project_id = ${definition.projectId},
            ...
            memory_context_mode = ${definition.memoryContextMode},
    ...
    execute: ({ now, limit }) =>
      sql`
        SELECT
          ...
          definitions.mode,
          COALESCE(definitions.memory_context_mode, 'off') AS "memoryContextMode",
  ...
  const createDefinition: AutomationRepositoryShape["createDefinition"] = (request) => {
    ...
    const definition: AutomationDefinition = {
      ...
      mode,
      memoryContextMode: input.memoryContextMode ?? "off",
    ...
    return insertDefinition({
      ...
      memoryContextMode: definition.memoryContextMode ?? "off",
